...
case class CharCooldown(@jsonField("cooldown_expiration") expires: OffsetDateTime /*, @jsonField("cooldown") seconds: Long */)
...
object Character {
  ...
  private def levelDecoder(prefix: String): JsonDecoder[Level] =
    ...
    JsonDecoder[Json].mapOrFail(json => for {
  ...
  private val skillsDecoder = allLevels.map(skills =>
    val (((((((mining, woodcutting), fishing), weaponcrafting), gearcrafting), jewelrycrafting), cooking), alchemy) = skills
    Skills(mining, woodcutting, fishing, weaponcrafting, gearcrafting, jewelrycrafting, cooking, alchemy)
  )
  implicit val jsonDecoder: JsonDecoder[Character] = JsonDecoder[Json].mapOrFail(json => for {
